...
function Navbar() {
  ...
  window.addEventListener("resize", () => {
    if (window.innerWidth < 1024 && screenSize >= 1024) {
      setScreenSize(window.innerWidth);
    } else if (window.innerWidth >= 1024 && screenSize < 1024) {
      setScreenSize(window.innerWidth);
    }
  });
  const show = () => {
    if (event.target.checked) {
      itemBox.current.style.right = "0";
    } else {
      itemBox.current.style.right = "-50%";
    }
  };
  const linkClicked = () => {
    itemBox.current.style.right = "-50%";
  ...
  const navbarItem = ["Services", "Our work", "About us", "Insights"];
  return (
    <Headroom>
      <div className={Styles["navbar"]}>
        ...
        >
          <div className={Styles["mainItems"]}>
            {navbarItem.map((item, index) => (
              <div className={Styles["relativePos"]} key={index}>
                <a href="#" onClick={screenSize < 1024 ? linkClicked : null}>
                  {item}
                </a>
                {screenSize >= 1024 && (
                  <div className={Styles["absPos"]} key={index}>
                    <a href="#" key={index}>
                      {item}
                    </a>
                  </div>
        ...
        <div className={Styles["hambargerMenu"]}>
          <input type="checkbox" onClick={show} ref={checkbox} />
